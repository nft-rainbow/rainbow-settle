...
func settle() error {
	...
	for k, v := range userCounts {
		...
		logrus.WithField("cost type", costType).WithField("price", price).WithField("count", count).WithField("in reset", countInResetQuota).WithField("in rollover", countInRolloverQuota).WithField("in balance", countInBalance).Debug("calculated cost quota")
